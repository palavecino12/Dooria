...
const InputFormUser = ({ name, label, control, type, error, options }: props) => {
    return (
        ...
        <Controller name={name} control={control} render={({ field }) =>
            type === 'select' ? (
                <select id={name} {...field} value={field.value ?? ""} className={`w-full p-4 rounded-xl border bg-white text-gray-700 
                ...
                <input id={name} type={type} {...field} value={field.value ?? ""} className={`w-full p-4 rounded-xl border bg-white text-gray-700 
                                                                                    shadow-lg focus:-translate-y-1 focus:shadow:2xl focus:outline-none 
                                                                                    transition-all duration-200
                                                                                    ${error ? 'border-red-600' : 'border-black/20'}`}/>
            )}
